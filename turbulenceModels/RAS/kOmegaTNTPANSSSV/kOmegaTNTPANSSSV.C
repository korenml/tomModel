...
void kOmegaTNTPANSSSV<BasicTurbulenceModel>::correctNut()
{
...
template<class BasicTurbulenceModel>
kOmegaTNTPANSSSV<BasicTurbulenceModel>::kOmegaTNTPANSSSV
(
...
template<class BasicTurbulenceModel>
bool kOmegaTNTPANSSSV<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void kOmegaTNTPANSSSV<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
	...
	// inverse fp , fpINV=1/fp=(1/fk)*(1-omegaU/(kU*PU))+omegaU/(kU*PU)
	volScalarField fpINV
	(
		(1/fk_)*(1.0-kU_*omegaU_/G)+kU_*omegaU_/G
	);
    ...
    // Scale-supplying resolved turbulent kinetic energy equation
    ...
    (
     ...
     ==
        (1.0-fk_)*(alpha*rho*fpINV*G)
      - fvm::SuSp((2.0/3.0)*alpha*rho*divU, kSSV_)
      - fvm::Sp((1.0-fk_)*Cmu_*alpha*rho*omegaU_, kSSV_)
